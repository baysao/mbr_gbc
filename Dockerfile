...
&& cd /tmp/openresty-${RESTY_VERSION} \
    && git clone https://github.com/Taymindis/nginx-link-function \
&& cp ./nginx-link-function/src/ngx_link_func_module.h /usr/include/ \
...
&& git clone https://github.com/vozlt/nginx-module-vts.git \
    && git clone https://github.com/baysao/ngx_stream_dns_proxy_module.git \
    && git clone https://github.com/leev/ngx_http_geoip2_module.git \
    && git clone --recursive https://github.com/maxmind/libmaxminddb \
    ; apk add --no-cache autoconf libtool automake \
...
--add-dynamic-module=./nginx-link-function \
--add-dynamic-module=./nginx-module-sts \
--add-dynamic-module=./nginx-module-stream-sts \
--add-dynamic-module=./nginx-module-vts \
--add-dynamic-module=./ngx_stream_dns_proxy_module \
...
echo "End build openresty" \
&& echo "GBC" \
    && bash -x /tmp/gbc/gbc_alpine.sh /tmp/gbc /tmp/gbc \
&& rm -rf /tmp/gbc/build /tmp/gbc/.git \
&& apk del .build-deps
